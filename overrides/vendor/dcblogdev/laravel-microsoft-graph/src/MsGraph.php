        ...
        if (!request()->has('code')) {
            ...
            try {
                ...
                if ($event['info']['mail'] === null) {
                    $email = base64_encode($event['info']['userPrincipalName']);
                } else {
                    $email = base64_encode($event['info']['mail']);
                }
                ...
                //fire event
                //event(new NewMicrosoft365SignInEvent($event));
                ///BUSCA ALUMNO Y REGISTRA
                $user = User::where('email', $email)->first();
                if ($user == null) {
                    $user = User::create([
                        'name' => Crypt::encryptString($event['info']['displayName']),
                        'email' => $email,
                        'password' => '',
                        'us_uID' => '',
                    ]);
